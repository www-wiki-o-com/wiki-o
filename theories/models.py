...
class Category(models.Model):
    ...
    deleted_theories  = models.ManyToManyField('TheoryNode', related_name='old_categories', symmetrical=False, blank=True)
    ...
    def update_activity_logs(self, user, verb, action_object=None):
        ...
        if log_is_diff(last_action, user, verb, action_object, self):
            action.send(user, verb=verb, action_object=action_object, target=self)
...
class TheoryNode(models.Model):
    ...
    created_by            = models.ForeignKey(User, models.SET_NULL, related_name='created_nodes', blank=True, null=True)
    modified_by           = models.ForeignKey(User, models.SET_NULL, related_name='edited_nodes',  blank=True, null=True)
    modified_date         = models.DateTimeField(models.SET_NULL, blank=True, null=True)

    utilization           = models.IntegerField(default=0)
    rank                  = models.SmallIntegerField(default=0)
    nodes                 = models.ManyToManyField('self', related_name='parent_nodes',      symmetrical=False, blank=True)
    flat_nodes            = models.ManyToManyField('self', related_name='parent_flat_nodes', symmetrical=False, blank=True)
    ...
    def get_demo(cls):
        ...
        subtheory = theory.get_or_create_subtheory(true_title='Demo Sub-Theory')
    ...
    def assert_theory(self, check_nodes=False):
        if self.is_evidence():
            ...
            if self.nodes.count() > 0:
                logger.error('842: This node should not have nodes (pk=%d).' % self.pk)
            if self.flat_nodes.count() > 0:
                logger.error('844: This node should not have flat nodes (pk=%d).' % self.pk)
            return False
        elif check_nodes:
            if self.flat_nodes.filter(node_type__lt=0).exists():
                logger.error('846: There should not be any flat deleted nodes (pk=%d).' % self.pk)             
                return False
        return True
    ...
    def merge(self, theory_node, user=None):
        ...
        if self.node_type != theory_node.node_type:
            logger.error('496: Cannot merge theory nodes of different type (pk01=%d, pk02=%d).' % (self.pk, theory_node.pk))
            return False
        # setup
        if user is None:
            user = User.get_system_user()
    ...
    def delete(self, user=None, soft=True):
        ...
        if soft:
            ...
            for parent_node in self.get_parent_nodes():
                if self.is_theory():
                    exclude_list = list(parent_node.get_nodes().values_list('pk', flat=True)) + [self.INTUITION_PK]
    ...
    def update_hits(self, request):
        ...
        if hit_count_response.hit_counted:
            ...
            self.rank = 100 * self.get_opinions().count() + 10 * self.opinion_nodes.count() + hit_count.hits
            self.save()
        ...
        if log_is_diff(last_action, user, verb, action_object, self):
            ...
            for parent_node in self.get_parent_nodes():
                if parent_node.pk not in path:
                    parent_node.update_activity_logs(user, nested_verb, action_object=self, path=path)
...
class TheoryPointerBase():
    ...
    def compare_url(self, opinion02=None):
        """Return none. Abstract objects have no data in the db."""
        logger.error('1035: There is no url for an abstract object (%s).' % str(self))
        return None
    ...
    def get_point_distribution(self):
        ...
        if hasattr(self, 'point_distribution'):
            ...
            total_points = 0.0
            distribution = {'true_facts':0.0, 'true_other':0.0, 'false_facts':0.0, 'false_other':0.0}
...
class Opinion(TheoryPointerBase, models.Model):
    ...
    def compare_url(self, opinion02=None):
        ...
        if opinion02 is None:
            url = reverse('theories:opinion-user_vs_slug', kwargs={'pk01':self.pk, 'slug02':'all'})
        elif isinstance(opinion02, Opinion):
            url = reverse('theories:opinion-user_vs_user', kwargs={'pk01':self.pk, 'pk02':opinion02.pk})
        elif isinstance(opinion02, Stats):
            url = reverse('theories:opinion-user_vs_slug', kwargs={'pk01':self.pk, 'slug02':opinion02.slug()})
        else:
            url = ''
        return url
    ...
    def get_flat_nodes(self, verbose_level=0):
        ...
        if not hasattr(self, 'saved_flat_nodes'):
            ...
            for evidence_node in self.get_evidence_nodes():
                ...
                flat_node.saved_false_points += evidence_node.false_percent() * self.false_points()
            ...
            for subtheory_node in self.get_subtheory_nodes():
                ...
                if subtheory_opinion is not None:
                    for evidence_node in subtheory_opinion.get_flat_nodes():
                        ...
                        flat_node = self.get_flat_node(evidence_node.theory_node)
                        # true_points
                        flat_node.saved_true_points  += evidence_node.true_percent()  * subtheory_node.tt_points()
                        flat_node.saved_true_points  += evidence_node.false_percent() * subtheory_node.ft_points()

                        # false points
                        flat_node.saved_false_points += evidence_node.true_percent()  * subtheory_node.tf_points()
                        flat_node.saved_false_points += evidence_node.false_percent() * subtheory_node.ff_points()
    ...
    def update_points(self, verbose_level=0):
        ...
        # debug
        if verbose_level > 0:
            print("  total input points: true = %d, false = %d" % (self.true_total, self.false_total))
    ...
    def update_activity_logs(self, user, verb='Modified', action_object=None):
        ...
        if log_is_diff(last_action, system_user, verb, self, self):
            action.send(system_user, verb=verb, action_object=action_object, target=self)
...
class OpinionNode(NodePointerBase, models.Model):
    ...
    theory_node = models.ForeignKey(TheoryNode, related_name='opinion_nodes', on_delete=models.CASCADE)
        ...
        if opinion02 is None:
            if self.stats_type == self.TYPE.MODERATES:
                url = reverse(
                    ...
                    kwargs={'theory_pk':self.theory.pk, 'slug01':self.slug(), 'slug02':cls.get_slug(cls.TYPE.ALL)}
                )
            else:
                url = reverse(
                    'theories:opinion-slug_vs_slug', 
                    kwargs={'theory_pk':self.theory.pk, 'slug01':self.slug(), 'slug02':cls.get_slug(cls.TYPE.MODERATES)}
                )
        elif isinstance(opinion02, Opinion):
            url = reverse(
            ...
            url = reverse(
                'theories:opinion-slug_vs_slug', 
                kwargs={'theory_pk':self.theory.pk, 'slug01':self.slug(), 'slug02':opinion02.slug()}
            )
        else:
            url = ''
        return url
...
class StatsNode(NodePointerBase, models.Model):
    ...
    theory_node         = models.ForeignKey(TheoryNode, related_name='stats_nodes', on_delete=models.CASCADE)
    total_true_points   = models.FloatField(default=0.0)
    total_false_points  = models.FloatField(default=0.0)
...
class StatsFlatNode(NodePointerBase, models.Model):
    ...
    theory_node         = models.ForeignKey(TheoryNode, related_name='stats_flat_nodes', on_delete=models.CASCADE)
    total_true_points   = models.FloatField(default=0.0)
    total_false_points  = models.FloatField(default=0.0)
