...
def get_or_create_theory(true_title, false_title=None, created_by=None, category='all'):
    ...
    kwargs = {'node_type': Content.TYPE.THEORY, 'title01': true_title}
...
def get_or_create_evidence(parent_theory, title, created_by=None, fact=False):
    ...
    if not parent_theory.assert_theory():
        return None
    # Get or create.
    kwargs = {'node_type': Content.TYPE.EVIDENCE, 'title01': title}
    if fact:
...
@nottest
def create_test_subtheory(parent_content, title='Sub-Theory', created_by=None, backup=False):
    """
    Create a test sub-theory using the input data.

    @details    Primarily used for unit tests.
    @param[in]  parent_content: The Content that is to be this theory's parent.
...
@nottest
def create_test_evidence(parent_content,
                         title='Evidence',
                         fact=False,
                         created_by=None,
                         backup=False):
...
@nottest
def create_test_opinion(content, user, true_input=None, false_input=None, force=False, nodes=False):
    """
    ...
    if nodes:
        random.seed(0)
        for child_content in content.get_nodes():
...
class UtilsTests(TestCase):
    ...
    def test_get_or_create_subtheory01(self):
        node01 = get_or_create_subtheory(self.content,
                                         true_title='Test',
                                         false_title='Test',
                                         created_by=self.user)
        node02 = get_or_create_subtheory(self.content, true_title='Test')
        self.assertIn(node01, self.content.get_nodes())
        self.assertEqual(node01, node02)
    ...
    def test_get_or_create_evidence01(self):
        node01 = get_or_create_evidence(self.content, title='Test', created_by=self.user)
        node02 = get_or_create_evidence(self.content, title='Test')
        self.assertIn(node01, self.content.get_nodes())
        self.assertEqual(node01, node02)
        self.assertNotIn(node01, self.category.get_theories())
