...
class ViewsTestBase():
    ...
    def create_data(self, user=None, created_by=None):
        ...
        self.evidence     = create_test_evidence(parent_theory=self.subtheory, created_by=created_by, backup=True)
        self.fact         = create_test_evidence(parent_theory=self.theory, title='Fact',    fact=True,  created_by=created_by)
        self.fiction      = create_test_evidence(parent_theory=self.theory, title='Fiction', fact=False, created_by=created_by)
    ...
    def test_get_theory_inherit(self, override=False, redirect_url=None, code=200):
        test_url = reverse('theories:theory-inherit', kwargs={'pk02':self.theory.pk, 'pk01':self.subtheory.pk})
        self.verify_get_response(test_url, redirect_url=redirect_url, code=code)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_evidence_merge(self, override=False, redirect_url=None, code=200):
        test_url = reverse('theories:evidence-merge', kwargs={'pk':self.evidence.pk}) + '?path=%d' % self.subtheory.pk
        self.verify_get_response(test_url, redirect_url=redirect_url, code=code)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_opinion_detail(self, override=False):
        test_url = reverse('theories:opinion-detail', kwargs={'pk':self.bobs_opinion.pk})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_opinion_slug(self, override=False):
        test_url = reverse('theories:opinion-slug', kwargs={'pk':self.theory.pk, 'slug':'all'})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_user_vs_user(self, override=False):
        test_url = reverse('theories:opinion-user_vs_user', kwargs={'pk01':self.bobs_opinion.pk, 'pk02':self.bobs_opinion.pk})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_user_vs_slug(self, override=False):
        test_url = reverse('theories:opinion-user_vs_slug', kwargs={'pk01':self.bobs_opinion.pk, 'slug02':'all'})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_slug_vs_user(self, override=False):
        test_url = reverse('theories:opinion-slug_vs_user', kwargs={'slug01':'all', 'pk02':self.bobs_opinion.pk})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_get_slug_vs_slug(self, override=False):
        test_url = reverse('theories:opinion-slug_vs_slug', kwargs={'theory_pk':self.theory.pk ,'slug01':'all', 'slug02':'all'})
        self.verify_get_response(test_url, code=200)
        # method must be overide
        self.assertTrue(override)
    ...
    def test_post_theory_inherit(self, override=False, redirect_url=None, code=302, modified=True):
        ...
        # test response
        test_url  = reverse('theories:theory-inherit', kwargs={'pk02':self.theory.pk, 'pk01':self.theory.pk})       
        post_data = {'form-0-select':True}
        self.verify_post_response(test_url, redirect_url, post_data, code)
    ...
    def test_post_theory_convert01(self, override=False, redirect_url=None, code=302, modified=True):
        # test response
        test_url  = reverse('theories:theory-convert', kwargs={'pk':self.subtheory.pk}) + '?path=%d' % self.theory.pk
    ...
    def test_post_theory_revert(self, override=False, redirect_url=None, code=302, modified=True):
        ...
        test_url  = reverse('theories:theory-revert', kwargs={'pk':self.theory.pk, 'vid':version.pk})
        self.verify_post_response(test_url, redirect_url, None, code)
    ...
    def test_post_evidence_merge(self, override=False, redirect_url=None, code=302, modified=True):
        ...
        # test response
        test_url  = reverse('theories:evidence-merge', kwargs={'pk':self.fiction.pk}) + '?path=%d' % self.theory.pk
        post_data = {'form-0-select':True}
        self.verify_post_response(test_url, redirect_url, post_data, code)
    ...
    def test_post_evidence_delete(self, override=False, redirect_url=None, code=302, deleted=True):
        ...
        # test response
        test_url = reverse('theories:evidence-delete', kwargs={'pk':self.evidence.pk}) + '?path=%d' % self.subtheory.pk        
        self.verify_post_response(test_url, redirect_url, None, code)
    ...
    def test_post_evidence_convert(self, override=False, redirect_url=None, code=302, modified=True):
       # test response
        test_url  = reverse('theories:evidence-convert', kwargs={'pk':self.evidence.pk}) + '?path=%d' % self.subtheory.pk
        self.verify_post_response(test_url, redirect_url, None, code)
    ...
    def test_post_evidence_revert(self, override=False, redirect_url=None, code=302, modified=True):
        ...
        test_url  = reverse('theories:evidence-revert', kwargs={'pk':self.evidence.pk, 'vid':version.pk}) + '?path=%d' % self.subtheory.pk
        self.verify_post_response(test_url, redirect_url, None, code)
    ...
    def test_post_opinion_new(self, override=False, redirect_url=None, code=302, modified=True):
        ...
        if modified:
            ...
            redirect_url = reverse('theories:opinion-detail', kwargs={'pk':opinion.pk})
            self.verify_redirect(response, redirect_url)
        else:
    ...
    def test_post_opinion_edit(self, override=False, redirect_url=None, code=302, modified=True):
        # setup
        if redirect_url is None:
            redirect_url = reverse('theories:opinion-detail', kwargs={'pk':self.my_opinion.pk})
    ...
    def test_post_delete_my_opinion(self, override=False, redirect_url=None, code=302, deleted=True):
        # setup
        if redirect_url is None:
            redirect_url = reverse('theories:theory-detail', kwargs={'pk':self.my_opinion.theory.pk})
    ...
    def test_post_delete_bobs_opinion(self, override=False, redirect_url=None, code=403, deleted=False):
        # setup
        if redirect_url is None:
            redirect_url = reverse('theories:theory-detail', kwargs={'pk':self.bobs_opinion.theory.pk})
