...
class EvidenceShape(SpringShape):
    ...
    def __init__(self, node, x, y, area, colour='black', opacity=1.0):
        ...
        r = (self.l/2) * math.sqrt(2)      # special relation for 45deg triangles
...
class Circle():
    ...
    def svg(self, offset={'x':0, 'y':0}):
        ...
        svg  = '<circle cx="%d" cy="%d" r="%d" fill="%s" fill-opacity="%0.2f" stroke="black" stroke-width="2.0"/>' % \
                (offset['x'] + self.x, offset['y'] + self.y, self.r, self.colour, self.opacity)
        return svg
...
class Rectangle():
    ...
    def svg(self, offset={'x':0, 'y':0}):
        """Output the svg code for shape."""
        svg  = '<rect x="%d" y="%d" width="%d" height="%d"' % (self.x01, self.y01, self.x02-self.x01, self.y02-self.y01)
        if self.hatch:
            svg += ' style="fill: url(#hatch)"'
        else:
            svg += ' fill="%s"' % self.colour
        svg += ' stroke="%s" fill-opacity="%0.2f" stroke-width="%0.2f"/>' % (self.stroke_colour, self.opacity, self.stroke_width)
        return svg
...
class Polygon():
    ...
    def svg(self, offset={'x':0, 'y':0}):
        """Output the svg code for shape."""
        svg  = '<path fill="%s" fill-opacity="%0.2f" stroke="black" stroke-width="1.0"' % (self.colour, self.opacity)
        svg += ' d="'
...
class Arrow():
    ...
    def svg(self, offset={'x':0, 'y':0}):
        ...
        svg  = '<path fill="none" stroke="%s" stroke-width="%0.2f"' % (self.colour, self.width)
        svg += ' d="'
...
class PieChart():
    ...
    def __init__(self, data):
        ...
        R = self.RADIUS
        true_text   = '%d' % int(round(100 * (self.data['true_facts']  + self.data['true_other'])))
        true_text  += '% True Points'
        false_text  = '%d' % int(round(100 * (self.data['false_facts'] + self.data['false_other'])))
    ...
    def create_graph(self, data=None, offset={'x':0, 'y':0}):
        ...
        if data['true_other'] > 0.001:
            if data['true_other'] > 0.999:
                ...
                w = Wedge(x, y, theta00, theta01, R, c_offset=C_OFFSET, colour='black', opacity=OPACITY)
        ...
        if data['true_facts'] > 0.001:
            ...
            if data['true_facts'] > 0.999:
                ...
                w = Wedge(x, y, theta01, theta02, R, c_offset=C_OFFSET, colour='black')
                self.shapes.append(w)
        ...
        if data['false_facts'] > 0.001:
            ...
            if data['false_facts'] > 0.999:
                ...
                w = Wedge(x, y, theta01, theta02, R, c_offset=C_OFFSET, colour='red')
                self.shapes.append(w)
        ...
        if data['false_other'] > 0.001:
            ...
            if data['false_other'] > 0.999:
                ...
                w = Wedge(x, y, theta01, theta02, R, c_offset=C_OFFSET, colour='red', opacity=OPACITY)
                self.shapes.append(w)
#        assert abs(theta02 - 360 - theta00) < 0.1
    ...
    def create_ledgend(self, x=0, y=0, text=None, colour='black'):
        ...
        # facts
        x01 = x-60; y01 = y
        self.shapes.append(Text('Facts', x=x01, y=y01-2*L, colour=colour, bold=True))
        self.shapes.append(Rectangle(x01-L, y01-L, x01+L, y01+L, colour=colour, opacity=1.0))

        # facts
        x01 = x+60; y01 = y
        self.shapes.append(Text('Other', x=x01, y=y01-2*L, colour=colour, bold=True))
        self.shapes.append(Rectangle(x01-L, y01-L, x01+L, y01+L, colour=colour, opacity=OPACITY))

        # points
        if text is not None:
            self.shapes.append(Text(text, x=x, y=y+R, size=40, colour=colour, bold=True))
...
class BarGraph():
        ...
        for i, h in enumerate(self.data[0]):
            if h > 0:
                ...
                if x01 <= 0:
                    ...
                    colour = 'red'
                self.shapes.append(Rectangle(x00*W+GAP, y00, x01*W-GAP, y00-h/max_h*H, colour=colour, stroke_colour='none'))
    ...
    def create_ledgend(self):
        ...
        # bottom boarder
        self.shapes.append(Rectangle(-W/2-20, y00, W/2+20, y00+3, opacity=1.0, colour='black'))
        # Tics
        x00 = -W/2; y01 = y00+10; dy=30
    ...
    def create_ledgend02(self):
        ...
        # bottom boarder
        self.shapes.append(Rectangle(-W/2-40, y00, W/2+40, y00+3, opacity=1.0, colour='black'))
        # true tic
        x00 = -W/2
        self.shapes.append(Text('100', x=x00, y=y00+50, colour='black', bold=True))
        self.shapes.append(Text('%',   x=x00+22, y=y00+50, colour='black', bold=True, align='start'))
        self.shapes.append(Rectangle(x00-GAP/2, y00+7, x00+GAP/2, y00+15, opacity=1.0, colour='black'))
        # mid tic
        x00 = 0
        self.shapes.append(Text('50', x=x00-10, y=y00+50, colour='black', bold=True, align='end'))
        self.shapes.append(Text('/',  x=x00,    y=y00+50, colour='black', bold=True, align='middle'))
        self.shapes.append(Text('50', x=x00+10, y=y00+50, colour='red',   bold=True, align='start'))
        self.shapes.append(Text('%',  x=x00+40, y=y00+50, colour='red',   bold=True, align='start'))
        self.shapes.append(Rectangle(x00-GAP/2, y00+7, x00+GAP/2, y00+15, opacity=1.0, colour='black'))
        # false tic
        x00 = W/2
        self.shapes.append(Text('100', x=x00, y=y00+50, colour='red', bold=True))
        self.shapes.append(Text('%',   x=x00+22, y=y00+50, colour='red', bold=True, align='start'))
        self.shapes.append(Rectangle(x00-GAP/2, y00+7, x00+GAP/2, y00+15, opacity=1.0, colour='black'))
        # True/False
        x01 = -W/2 - 100; y01 = H 
        self.shapes.append(Text('True',  x=x01,  y=y01, size=40, colour='black', bold=True))
        self.shapes.append(Text('False', x=-x01, y=y01, size=40, colour='red', bold=True))
        # Supporters - Moderates - Opposers
        x01 = -W/6-W/6; y01 = y00+90
        self.shapes.append(Text('Supporters', x=x01, y=y01, size=30, bold=True))
        x01 = 0;
        self.shapes.append(Text('Moderates',  x=x01, y=y01, size=30, bold=True))
        x01 = W/6+W/6;
        ...
        # Tics
        x01 = -W/2;
        self.shapes.append(Rectangle(x01-GAP/4, y01-15, x01+GAP/4, y01, opacity=1.0, colour='black'))
...
class OpinionVennDiagram():
    ...
    def output_svg(self):
        ...
        height  = (2.0*R + BOARDER['top'] + BOARDER['bottom'])
        width   = 1200
        offset  = {'x':width/2 - (self.true_ring.x + self.false_ring.x)/2, 'y':BOARDER['top'] + R - self.true_ring.y}
    ...
    def create_boundary_shapes(self):
        ...
        self.in_boundry_shapes.append(Ring((X_NEG + X_POS)/2, self.true_ring.y, X_WID/2))
    ...
    def fix_overlap01(self):
        ...
        if len(self.int_shapes) > 0:
            shapes.append({'prop':self.true_shapes,   'in':[self.true_ring], 'out':self.true_shapes+[self.false_ring]})
            shapes.append({'prop':self.false_shapes,  'in':[self.false_ring], 'out':self.false_shapes+[self.true_ring]})
            shapes.append({'prop':self.int_shapes,    'in':[self.true_ring, self.false_ring], 'out':self.int_shapes})
            shapes.append({'prop':[self.true_ring],   'in':self.true_shapes + self.int_shapes, 'out':self.false_shapes})
            shapes.append({'prop':[self.false_ring],  'in':self.false_shapes + self.int_shapes, 'out':self.true_shapes})
        else:
            shapes.append({'prop':self.true_shapes,   'in':[self.true_ring], 'out':self.true_shapes+[self.false_ring]})
            shapes.append({'prop':self.false_shapes,  'in':[self.false_ring], 'out':self.false_shapes+[self.true_ring]})
        # propagate
    ...
    def fix_overlap02(self):
        ...
        # setup
        shapes = [{'prop':self.out_shapes, 'in':self.in_boundry_shapes, 'out':self.out_boundry_shapes + self.out_shapes}]
            ...
            for shape_set in shapes:
                for shape01 in shape_set['prop']:
                    ...
                    for shape02 in shape_set['out']:
                        if shape02 != shape01:
                            fx, fy = shape01.get_force(shape02, direction='out')
                            total_x_force += fx
                            total_y_force += fy
    ...
    def create_ledgend(self):
        ...
        R = self.RADIUS
        BOARDER = self.BOARDER
        self.text.append(Text('True',  x=self.true_ring.x,  y=self.true_ring.y - 1.0*13/12*R, size=40, colour='black', bold=True))
        self.text.append(Text('False', x=self.false_ring.x, y=self.true_ring.y - 1.0*13/12*R, size=40, colour='red',   bold=True))
        if self.bottom_text is not None:
            self.text.append(Text(self.bottom_text,  x=(self.true_ring.x + self.false_ring.x)/2, y=0.95*R + BOARDER['bottom']))
...
#        seed = 72
        ...
        for node in opinion.saved_nodes:
            if total_true_points  > 0:
                node.saved_true_points  = (node.true_points() / total_true_points)   * opinion.true_points() 
            else:
                node.saved_true_points  = 0.0
            if total_false_points > 0:
                node.saved_false_points = (node.false_points() / total_false_points) * opinion.false_points()
            else:
