...
def NotificationsView(request):
    ...
    ViolationFormset    = modelformset_factory(Violoation,   form=SelectVioloationForm,   extra=0)
    ...
    if request.method == 'POST':
        ...
        formset01 = NotificationFormset(request.POST, queryset=notifications.object_list, prefix='notifications')
        formset02 = ViolationFormset   (request.POST, queryset=user_feedback.object_list, prefix='feedback')
    ...
    # GET request
        ...
        formset02 = ViolationFormset   (queryset=user_feedback.object_list, prefix='feedback')
...
def ViolationResolveView(request, pk):
    ...
    if content_type == 'theorynode':
        ...
        revisions           = theory_node.get_revisions().filter(revision__date_created__date__lte=violation.pub_date)
        RevisionFormSet     = modelformset_factory(Version, form=TheoryRevisionForm, extra=0)
        revision_formset    = RevisionFormSet(queryset=revisions, form_kwargs={'user':user, 'hide_delete':True})
    else:
        theory_node         = None
        revision_formset    = []
    ...
    if request.method == 'POST':
        ...
        # vote
        vote_form = VoteForm(request.POST, instance=vote, violation=violation, user=user, prefix='vote')
        ...
        # report
        report_form = ReportViolationForm(request.POST, content=violation, user=user, prefix='report')
        ...
        feedback_form = ResolveViolationForm(request.POST, violation=violation, user=user, prefix='feedback')
        ...
        if theory_node is not None:
            theorynode_form = TheoryForm(request.POST, instance=theory_node, user=user, prefix='theorynode')
