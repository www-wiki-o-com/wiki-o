...
class User(AbstractUser):
    ...
    contributions       = models.ManyToManyField('theories.TheoryNode', related_name='collaborators', blank=True)
...
class Violation(models.Model):
    ...
    def poll_is_done(self):
        if self.is_closed():
            ...
            end   = datetime.datetime(year=end.year, month=end.month, day=end.day, hour=23, minute=59, second=59)
            return today > end
...
class ViolationFeedback(models.Model):
    ...
    user              = models.ForeignKey(User, related_name='violation_feedback', on_delete=models.SET_NULL, null=True)
...
class ViolationVote(models.Model):
    ...
    user              = models.ForeignKey(User, related_name='violation_votes', on_delete=models.SET_NULL, null=True)
